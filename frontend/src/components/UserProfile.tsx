...
function formatDate(value?: string | null) {
  // Format ISO timestamp to readable date/time (e.g., 'Jan 12, 2026, 3:45 PM'); fallback 'Never'.
...
export default function UserProfile({ token, userId, currentUser, isAdmin, onOpenJob, onOpenProfile, onCurrentUserUpdated }: Props) {
  // Fetched profile data: user record, time entries, work history, and stats.
  const [data, setData] = useState<UserProfileResponse | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  // Edit form state: user profile fields for first name, last name, email, password, role, disabled status.
  ...
  // Save operation state: submission, error, and success confirmation.
  const [saving, setSaving] = useState(false);
  const [saveError, setSaveError] = useState<string | null>(null);
  const [saveStatus, setSaveStatus] = useState<string | null>(null);
  // Permission checks: can only edit if admin or viewing self.
  ...
  async function handleSave(event: FormEvent<HTMLFormElement>) {
    ...
    // Build a minimal patch so unchanged values do not trigger unnecessary writes.
    const payload: {
      firstName?: string;
      lastName?: string;
      email?: string;
